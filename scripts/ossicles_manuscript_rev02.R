...
# Loading Dataset
raw <- read_xlsx("data/Supplemental_Tables_rev02.xlsx", na = "S1", skip = 1)
...
test <- data %>%
  ...
  select(Sample, Bone, `Library ID`, `Human Proportion (%)`, `Clonality (%)`, 
         `Avg Read Length (bp)`, `PMD (%)`, `Cont. Est.`, `mt Coverage (x)`)%>%
  ungroup()

test_m <- melt(test, id.vars = c("Sample", "Bone", "Library ID", "mt Coverage (x)"))
...
stapes <- test_m %>%
  ...
  filter(!(variable == "Cont. Est." & is.na(diff))) %>%
...
write.table(seqstats, "data/seqstats.txt", sep = "\t", row.names = F, quote = F)
  ...
  # Microbial Analysis -------------------------------------------------------------
##### Comparison of Non-oral and Oral reads ----
raw <- read_xlsx("data/Supplemental_Tables_rev02.xlsx", sheet = "S2", skip = 1)
