...
ggsave("figures/svg/figure_2.svg", device = svg, width = 23, height = 15)
...
# PCoA analysis. Reading result files.
pcoa_df <- read.table("data/pcoa_df", header = T, sep = "\t")
pcoa_df$bone <- factor(pcoa_df$bone, levels = c("Petrous", "Stapes", "Malleus", "Incus", "Unidentified Ossicle"))

top_species <- read.table("data/top_species", header = T, sep = "\t")
permanova_results <- read.table("data/permanova_results", header = T, sep = "\t")
...
# Filter result files
pcoa_df_anatolia <- pcoa_df %>%
  filter(region %in% "Anatolia")

top_species_anatolia <- top_species %>%
  filter(region %in% "Anatolia")


p1 <- ggplot(pcoa_df_anatolia, aes(x = PCoA1, y = PCoA2)) +
  xlim(-1, 1)+
  ylim(-1, 1)+
  geom_segment(data = top_species_anatolia,
               aes(x = 0, y = 0, xend = Dim1, yend = Dim2),
               arrow = arrow(length = unit(0.2, "cm")), color = "black", lwd = 0.7, alpha = .1)+
  geom_text(data = top_species_anatolia,
            aes(x = Dim1, y = Dim2, label = species),
            size = 3, fontface = "italic", vjust = -0.5, alpha = .2) +
  geom_point(aes(color = bone), size = 4) +
  scale_color_manual(values = c(col_p, col_s, col_m, col_i, col_ui, col_t))+
  theme_bw() +
  labs(
       x = paste0("PCoA1 (", 25.2, "%)"),
       y = paste0("PCoA2 (", 11.1, "%)"),
       caption = bquote(paste("PERMANOVA: ", F == 5.046, ", ", R^2 == 0.156, ", ", italic(p) == 0.003))) +
  theme(panel.grid.minor = element_blank(),
        ...
        plot.caption = element_text(size = 11));p1


##### Second Panel ----- 

# Filter result files
pcoa_df_iberia <- pcoa_df %>%
  filter(region %in% "Iberia")

top_species_iberia <- top_species %>%
  filter(region %in% "Iberia")

p2 <- ggplot(pcoa_df_iberia, aes(x = PCoA1, y = PCoA2)) +
  xlim(-1, 1)+
  ylim(-1, 1)+
  geom_segment(data = top_species_iberia,
               aes(x = 0, y = 0, xend = Dim1, yend = Dim2),
               arrow = arrow(length = unit(0.2, "cm")), color = "black", lwd = 0.7, alpha = .1)+
  geom_text(data = top_species_iberia,
            aes(x = Dim1, y = Dim2, label = species),
            size = 3, fontface = "italic", vjust = -0.5, alpha = .15) +
  geom_point(aes(color = bone), size = 4) +
  scale_color_manual(values = c(col_p, col_s, col_m, col_i, col_ui, col_t))+
  theme_bw() +
  labs(
       x = paste0("PCoA1 (", 27.5, "%)"),
       y = paste0("PCoA2 (", 20.8, "%)"),
       caption = bquote(paste("PERMANOVA: ", F == 0.742, ", ", R^2 == 0.142, ", ", italic(p) == 0.504))) +
  theme(panel.grid.minor = element_blank(),
        axis.text = element_text(face = "bold", size = 11),
        axis.title = element_text(face = "bold", size = 11),
        plot.title = element_text(face = "bold", size = 12),
        ...
        legend.title = element_blank(),
        legend.text = element_text(face = "bold", size = 11),
        plot.caption = element_text(size = 11));p2


top_row <- ggarrange(p1, p2, common.legend = T, nrow = 1, ncol = 2, labels = c("A", "B"));top_row


##### Third Panel ----- 
# Comparion of Non-Oral and Oral relative abundances between bone types
...
pb_1 <- biplot %>%
  ...
  geom_point(aes(x = `Tissue Type`, y = `Non-oral`, fill = `Tissue Type`),
             shape = 21, size = 3, stroke = .75, position = position_jitter(width = .2))+
...
pb_2 <- biplot %>%
  ...
  geom_boxplot(aes(x = `Tissue Type`, y = Oral, fill = `Tissue Type`, color = `Tissue Type`), 
               outliers = FALSE, show.legend = F)+
  geom_point(aes(x = `Tissue Type`, y = Oral, fill = `Tissue Type`),
             shape = 21, size = 3, stroke = .75, position = position_jitter(width = .2))+
...
bottom_row <- ggarrange(pb_1, pb_2, ncol = 2, nrow = 1, labels = c("C", ""), common.legend = T);bottom_row

all <- ggarrange(top_row, bottom_row, nrow = 2, ncol = 1, common.legend = F, heights = c(1,.75));all

# Saving
ggsave("figures/svg/supplementary_figure_4.svg", device = svg, width = 15, height = 20)
ggsave("figures/png/supplementary_figure_4.png", device = "png", height = 15, width = 20, dpi = 600)
